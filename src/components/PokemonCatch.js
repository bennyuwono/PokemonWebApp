...
import "../assets/PokemonCatch.css";

const PokemonCatch = ({ pokemon, isPokemonCaught, pokemonDetail }) => {
  ...
  const validateNickname = () => {
    if (nameList.includes(nickname)) {
      document.getElementById("nicknameErrMessage").innerHTML =
        "Name already registered. Please choose another name";
    } else {
      setMyPokemon();
    }
  };
  ...
  return (
    <div className="catch-modal">
      ...
      {!isPokemonCaught && <p className="catch-modal__try-again">try again?</p>}
      {isPokemonCaught && (
        <>
          <img
            ...
            alt="Pokemon Sprites"
          />
          <p className="catch-modal__caption">
          ...
          {nickname.length > 0 && (
            <button onClick={validateNickname}>Confirm</button>
          )}
        </>
      )}
    </div>
